...
조직(`<조직 ID>`)에 전용 계정을 추가하고 그 안에만 배포한다.
관리 계정에는 **워크로드도 시크릿도 두지 않는다** — SSM 파라미터를 포함한 모든 런타임 자격정보는
`waganda` 계정에만 존재한다. 다른 계정에 두면 배포된 Lambda·AgentCore 가 읽지 못하고,
증상은 조용한 인증 실패로만 드러난다.

```
<조직 ID>  (Organizations, FeatureSet: ALL, SCP 활성)
├── <관리 계정 ID>       Yan So      관리 계정 — 워크로드·시크릿 없음, yanbert.com 호스팅 존 보유
├── <기존 분리 계정 ID>  iwasthere   기존 분리 계정
└── waganda                          ← 이 프로젝트의 모든 리소스
```
