...
import { z } from 'zod';
import { RegionPatch } from '@waganda/schemas';
...
import { deleteRegion, updateRegion } from '@/lib/services/wines';

const PatchBody = RegionPatch.extend({ rev: z.number().int().min(0) });

/** PATCH /api/regions/[id] — 지역 수정 (6.3). 편집자 가드 적용. */
export const PATCH = withEditorGuard(async (request: NextRequest) => {
  ...
  try {
    const region = await updateRegion(repo, id, rev, patch);
...
/** DELETE /api/regions/[id] — 지역 삭제 (6.3). 연결된 와인·와이너리·하위지역이 있으면 거부한다. */
